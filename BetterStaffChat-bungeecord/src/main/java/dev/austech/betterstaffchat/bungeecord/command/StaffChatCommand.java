...
public class StaffChatCommand extends Command {
    public StaffChatCommand() {
        super("staffchat", "betterstaffchat.messages.send", "sc");
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (sender instanceof ProxiedPlayer)
            StaffChatUtil.getInstance().discord(sender, ChatColor.stripColor(TextUtil.colorize(BetterStaffChatBungeeCord.getInstance().getConfig().getString("discord.discord-messages.staffchat-format").replace("%player_name%", sender.getName()).replace("%message%", String.join(" ", Arrays.copyOfRange(args, 0, args.length))))));
        else
            StaffChatUtil.getInstance().discord(sender, ChatColor.stripColor(TextUtil.colorize(BetterStaffChatBungeeCord.getInstance().getConfig().getString("discord.discord-messages.staffchat-format").replace("%player_name%",
                    BetterStaffChatBungeeCord.getInstance().getConfig().getString("staffchat.console-replacement")).replace("%message%", String.join(" ", Arrays.copyOfRange(args, 0, args.length))))));
    }
}
