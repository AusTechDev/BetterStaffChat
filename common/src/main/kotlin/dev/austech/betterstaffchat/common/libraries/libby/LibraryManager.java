...
import dev.austech.betterstaffchat.common.libraries.libby.relocation.RelocationHelper;
...
public abstract class LibraryManager {
   ...
   */
  private final Map<String, IsolatedClassLoader> isolatedLibraries =
      new HashMap<>();
   ...
   *
   * @deprecated Use {@link LibraryManager#LibraryManager(LogAdapter, Path,
   *     String)}
   */
  @Deprecated
  protected LibraryManager(LogAdapter logAdapter, Path dataDirectory) {
    logger = new Logger(requireNonNull(logAdapter, "logAdapter"));
    saveDirectory = requireNonNull(dataDirectory, "dataDirectory")
                        .toAbsolutePath()
                        .resolve("lib");
  }
  ...
  protected LibraryManager(LogAdapter logAdapter, Path dataDirectory,
    ...
    logger = new Logger(requireNonNull(logAdapter, "logAdapter"));
    saveDirectory =
        requireNonNull(dataDirectory, "dataDirectory")
            .toAbsolutePath()
            .resolve(requireNonNull(directoryName, "directoryName"));
  }
  ...
  protected void addToIsolatedClasspath(Library library, Path file) {
    ...
    if (id != null) {
      classLoader =
          isolatedLibraries.computeIfAbsent(id, s -> new IsolatedClassLoader());
    } else {
   ...
   * @return log level
   */
  public LogLevel getLogLevel() { return logger.getLevel(); }
   ...
   */
  public void setLogLevel(LogLevel level) { logger.setLevel(level); }
  ...
  public Collection<String> getRepositories() {
    List<String> urls;
    synchronized (repositories) { urls = new LinkedList<>(repositories); }
  ...
  public void addMavenLocal() {
    addRepository(Paths.get(System.getProperty("user.home"))
   ...
   */
  public void addMavenCentral() { addRepository(Repositories.MAVEN_CENTRAL); }

  /**
   * Adds the Sonatype OSS repository.
   */
  public void addSonatype() { addRepository(Repositories.SONATYPE); }

  /**
   * Adds the Bintray JCenter repository.
   */
  public void addJCenter() { addRepository(Repositories.JCENTER); }

  /**
   * Adds the JitPack repository.
   */
  public void addJitPack() { addRepository(Repositories.JITPACK); }
  ...
  public Collection<String> resolveLibrary(Library library) {
    Set<String> urls =
        new LinkedHashSet<>(requireNonNull(library, "library").getUrls());
  ...
  private byte[] downloadLibrary(String url) {
    try {
      URLConnection connection =
          new URL(requireNonNull(url, "url")).openConnection();
  ...
  public Path downloadLibrary(Library library) {
    ...
    Path file = saveDirectory.resolve(library.getArtifactId() + "-" +
    ...
    if (urls.isEmpty()) {
      throw new RuntimeException("Library '" + library +
                                 "' couldn't be resolved, add a repository");
    }
      ...
      for (String url : urls) {
        ...
        if (md != null) {
          ...
          if (!Arrays.equals(checksum, library.getChecksum())) {
            ...
            logger.warn(" URL :  " + url);
            logger.warn(" Expected :  " + Base64.getEncoder().encodeToString(
                                              library.getChecksum()));
            logger.warn(" Actual :  " +
   ...
   */
  private Path relocate(Path in, String out,
    ...
    try {
      ...
      logger.info("Relocations applied to " +
                  saveDirectory.getParent().relativize(in));
  ...
  public void loadLibrary(Library library) {
    ...
    if (library.hasRelocations()) {
      file =
          relocate(file, library.getRelocatedPath(), library.getRelocations());
    }
