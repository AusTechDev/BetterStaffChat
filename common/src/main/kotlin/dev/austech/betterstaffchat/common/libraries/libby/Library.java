...
public class Library {
   ...
   */
  private Library(Collection<String> urls, String id, String groupId,
                  String artifactId, String version, String classifier,
                  byte[] checksum, Collection<Relocation> relocations,
                  boolean isolatedLoad) {

    this(urls, null, id, groupId, artifactId, version, classifier, checksum,
         relocations, isolatedLoad);
  }
   ...
   * @param isolatedLoad isolated load for this library
   */
  private Library(Collection<String> urls, Collection<String> repositories,
                  String id, String groupId, String artifactId, String version,
                  String classifier, byte[] checksum,
                  Collection<Relocation> relocations, boolean isolatedLoad) {
    ...
    String path = this.groupId.replace('.', '/') + '/' + artifactId + '/' +
    ...
    this.repositories =
        repositories != null
            ? Collections.unmodifiableList(new LinkedList<>(repositories))
            : Collections.emptyList();
    relocatedPath =
        hasRelocations() ? artifactId + '-' + version + "-relocated.jar" : null;
   ...
   */
  public boolean hasClassifier() { return classifier != null; }
   ...
   */
  public boolean hasRelocations() { return !relocations.isEmpty(); }
   ...
   */
  public String getRelocatedPath() { return relocatedPath; }
  ...
  public static class Builder {
     ...
     * <p>Most common repositories can be found in {@link Repositories} class as
     * constants. <p>Note that repositories should be preferably added to the
     * {@link LibraryManager} via {@link LibraryManager#addRepository(String)}.
     *
    ...
    public Builder repository(String url) {
      repositories.add(
          requireNonNull(url, "repository").endsWith("/") ? url : url + '/');
      return this;
    }
    ...
    public Builder checksum(String checksum) {
      return checksum(
          Base64.getDecoder().decode(requireNonNull(checksum, "checksum")));
    }
