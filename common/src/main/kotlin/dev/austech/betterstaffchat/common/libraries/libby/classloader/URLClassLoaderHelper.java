package dev.austech.betterstaffchat.common.libraries.libby.classloader;
...
public class URLClassLoaderHelper {
  ...
  static {
    ...
    // getDeclaredField("theUnsafe") is not used to avoid breakage on JVMs with
    // changed field name
  ...
  /**
   * A reflected method in {@link URLClassLoader}, when invoked adds a URL to
   * the classpath.
   */
        ...
        if (exception.getClass().getName().equals(
          ...
          if (theUnsafe != null)
            try {
              ...
              addURLMethodHandle =
                  null; // Just to be sure the field is set to null
            }
          ...
          try {
            ...
            System.err.println(
                "Cannot access URLClassLoader#addURL(URL), if you are using Java 9+ try to add the following option to your java command: --add-opens java.base/java.net=ALL-UNNAMED");
            throw new RuntimeException(
                "Cannot access URLClassLoader#addURL(URL)", e);
          }
        } else {
          throw new RuntimeException(
              "Cannot set accessible URLClassLoader#addURL(URL)", exception);
        }
      }
      this.addURLMethodHandle =
          MethodHandles.lookup().unreflect(addURLMethod).bindTo(classLoader);
  ...
  private static void openUrlClassLoaderModule() throws Exception {
    //
    // Thanks to lucko (Luck) <luck@lucko.me> for this snippet used in his own
    // class loader
    //
    ...
    Method addOpensMethod =
        moduleClass.getMethod("addOpens", String.class, moduleClass);
    ...
    addOpensMethod.invoke(urlClassLoaderModule,
                          URLClassLoader.class.getPackage().getName(),
                          thisModule);
  }
  ...
  private MethodHandle getPrivilegedMethodHandle(Method method)
    ...
    // The Unsafe class is used to get a privileged MethodHandles.Lookup
    // instance.

    // Looking for MethodHandles.Lookup#IMPL_LOOKUP private static field
    // getDeclaredField("IMPL_LOOKUP") is not used to avoid breakage on JVMs
    // with changed field name
  ...
  private void addOpensWithAgent(LibraryManager libraryManager)
    ...
    // Try to add a java agent at runtime (specifically, ByteBuddy's agent) and
    // use it to open the module, since java agents should have such permission.
    ...
    try {
      ...
      Class<?> byteBuddyAgent =
          isolatedClassLoader.loadClass("net.bytebuddy.agent.ByteBuddyAgent");
      ...
      //     Collections.singletonMap("java.net",
      //     Collections.singleton(getClass().getModule())),
      //     Collections.emptySet(),
      //     Collections.emptyMap()
      // );
      //
      // For more information see
      // https://docs.oracle.com/en/java/javase/16/docs/api/java.instrument/java/lang/instrument/Instrumentation.html
      //
      // We use reflection since we build against Java 8.

      Object instrumentation =
          byteBuddyAgent.getDeclaredMethod("install").invoke(null);
      Class<?> instrumentationClass =
          Class.forName("java.lang.instrument.Instrumentation");
      Method redefineModule = instrumentationClass.getDeclaredMethod(
          "redefineModule", Class.forName("java.lang.Module"), Set.class,
          Map.class, Map.class, Set.class, Map.class);
      Method getModule = Class.class.getDeclaredMethod("getModule");
      Map<String, Set<?>> toOpen = Collections.singletonMap(
          "java.net", Collections.singleton(getModule.invoke(getClass())));
      redefineModule.invoke(
          instrumentation, getModule.invoke(URLClassLoader.class),
          Collections.emptySet(), Collections.emptyMap(), toOpen,
